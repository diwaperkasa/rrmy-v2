<?php
// [blog_posts]
function shortcode_latest_from_blog($atts, $content = null, $tag = null) {
...
while ( $recentPosts->have_posts() ) : $recentPosts->the_post();
		...
		?>
	<?php if($repeater['type'] == 'slider-full') : ?>
		<div class="sp-slide">
			<div class="plain">
				<div class="box <?php echo $classes_box; ?> box-blog-post has-hover">
			        ...
			        <?php } ?>
					<div class="box-text <?php echo $classes_text; ?> <?php foreach((get_the_category()) as $cat) { echo 'border-'. strtolower(str_replace(' ', '-', preg_replace('/[^A-Za-z0-9\-]/', '', $cat->cat_name))) . ' '; } ?>" <?php echo get_shortcode_inline_css($css_args); ?>>
						...
						<?php
							foreach((get_the_category()) as $cat) {
								if( !$cat->parent ){
									echo '<a href="'. get_category_link( $cat->cat_ID ) .'"><span class="cat-'. strtolower(str_replace(' ', '-', preg_replace('/[^A-Za-z0-9\-]/', '', $cat->cat_name))) .'">'. $cat->cat_name . '</span></a> ';
								}
							}
							/*foreach((get_the_category()) as $cat) {
								if( $cat->parent ){
						...
						<?php } ?>
						<a href="<?php the_permalink() ?>"><h5 class="post-title is-<?php echo $title_size; ?> <?php echo $title_style;?>"><?php the_title(); ?></h5></a>
	...
	<?php else: ?>
		<div class="col <?php echo implode(' ', $col_class); ?>" <?php echo $animate;?>>
			<div class="col-inner <?php foreach((get_the_category()) as $cat) { echo 'border-'. strtolower(str_replace(' ', '-', preg_replace('/[^A-Za-z0-9\-]/', '', $cat->cat_name))) . ' '; } ?>">
				<div class="plain">
					<div class="box <?php echo $classes_box; ?> box-blog-post has-hover">
				        <?php if(has_post_thumbnail()) { ?>
		  					<div class="box-image" <?php echo get_shortcode_inline_css($css_args_img); ?>>
						...
						<a href="<?php the_permalink() ?>">
							<?php $strlen = strlen(get_the_title()); //echo $strlen; ?>
							<h5 class="post-title is-<?php echo $title_size; ?> <?php echo $title_style;?> <?php if($strlen > 80) { echo 'extra-text'; } ?>"><?php the_title(); ?></h5>
						</a>
						<?php if((!has_post_thumbnail() && $show_date !== 'false') || $show_date == 'text') {?>
							<div class="post-meta is-small op-8"><?php echo get_the_date(); ?> <span>- <?php echo esc_html( get_the_author_meta( 'display_name' ) ) ?></span></div>
						<?php } ?>
						<div class="is-divider"></div>
						<?php if($excerpt !== 'false') { ?>
						<p class="from_the_blog_excerpt <?php if($excerpt !== 'visible'){ echo 'show-on-hover hover-'.$excerpt; } ?>"><?php
...
if($repeater['type'] == 'slider-full') {
	...
	while ( $recentPosts->have_posts() ) : $recentPosts->the_post();
		...
		?>
		<div class="sp-thumbnail <?php foreach((get_the_category()) as $cat) { echo 'border-'. strtolower(str_replace(' ', '-', preg_replace('/[^A-Za-z0-9\-]/', '', $cat->cat_name))) . ' '; } ?>">
			<?php if($show_category !== 'false') { ?>
				<p class="cat-label <?php if($show_category == 'label') echo 'tag-label'; ?> is-xxsmall op-7 uppercase">
			<?php
				foreach((get_the_category()) as $cat) {
					if( !$cat->parent ){
						echo '<span class="cat-'. strtolower(str_replace(' ', '-', preg_replace('/[^A-Za-z0-9\-]/', '', $cat->cat_name))) .'">'. $cat->cat_name . '</span> ';
					}
				}
				/*foreach((get_the_category()) as $cat) {
					if( $cat->parent ){
